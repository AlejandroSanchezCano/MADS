...
class InterPro:
    ...
    # mamba create -n data python=3.10 tqdm requests numpy seaborn
    ...
    def save(interactors: list[Interactor]) -> None:
        ...
        for interactor in interactors:
            utils.pickling(data = interactor.__dict__, path = f'{var.INTERACTORS}/{interactor.uniprot_id}.int')

        # Logger
        logger.info(f'{len(interactors)} Interactor objects have been pickled')

    @staticmethod
    def get_sequences(interactors: list[Interactor]) -> None:
        '''
        Download sequences from the UniProt database of the list of MIKC proteins.

        Parameters
        ----------
        interactors : list[Interactor]
            List of Interactor objects to download sequences of.
        '''
        ...
        # Logging
        logger.info(f'{len(interactors)} sequences have been obtained form UniProt')

    @staticmethod
    def get_structures(interactors: list[Interactor]) -> None: #14106 out of 16990
        '''
        ...
        for interactor in tqdm(interactors):
            ...
            structure_path = f'{var.AF}/{interactor.uniprot_id}.pdb'
        ...
        # Logging
        n_pdb_files = len(os.listdir(var.AF))
...
if __name__ == '__main__':
    ...
    InterPro.get_sequences(interactors = mikc)
    InterPro.get_structures(interactors = mikc)