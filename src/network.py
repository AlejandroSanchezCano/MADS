...
class Network:
    ...
    def __init__(self, database = 'STRING', type = 'MADS vs MADS', species = 'all'):
        ...
        self.path = f'{var.NETWORKS}/{database}_{type}.pkl'
        self.df = utils.unpickling(self.path)
        self.df = self.df.drop_duplicates('A-B') # CAREFUL CAUSE WE LOSE INFO
        self.df = self._add_species()
        self.df = self._species(species) if species != 'all' else self.df
...
if __name__ == '__main__':
    n = Network(database = 'IntAct', type = 'MADS vs MADS', species = '3702')
    print(n.df)
    print(n.positive_pairs())